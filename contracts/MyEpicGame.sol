...
contract MyEpicGame is ERC721 {
  ...
  function attackBoss(uint nftTokenId) public {
    CharacterAttributes storage player = nftAttributes[nftTokenId];
    ...
    if(randomInt(10) >= 0) {
      if(player.hp < bigBoss.attackDamage) {
        player.hp = 0;
        console.log("%s is dead!", player.name);
      } else {
  ...
  function checkIfUserHasNFT() public view returns (CharacterAttributes[] memory characterNFT, uint256[] memory tokenId) {
    uint256[] memory userNftTokenIds = nftHolders[msg.sender];
    CharacterAttributes[] memory output = new CharacterAttributes[](userNftTokenIds.length);
    for(uint i = 0; i < userNftTokenIds.length; i++) {
      output[i] = nftAttributes[userNftTokenIds[i]];
    }
    return (output, userNftTokenIds);
  }
